// Deleted and re-added built-in sections retain Pi's canonical render order.
// Extractors and the sanitizer consume that reconstructed prompt; no capture registry is involved.
// Unknown sections retain their replay-relative position.
...
describe("toBridgeContext section replay order", () => {
	it("re-emits sections in canonical order after a delete-then-re-add reorders the Map", () => {
		...
		// Canonical rendering places skills before cwd regardless of patch order.
