...
- `appendSystemPrompt` — append pi's AGENTS.md and skills (default `true`). This covers pi's own content only. A short `# Harness corrections` block is always appended regardless, because Claude Code's preset prompt otherwise names tools and a model ID that do not exist here; see [diag/SYSTEM-PROMPTS.md](diag/SYSTEM-PROMPTS.md).
