...
7. Emit a one-time diagnostic when the installed catalog does not supply an ID listed in `MODEL_IDS_IN_ORDER`. `buildModels()` already knows which IDs it dropped, so the check is a length or set comparison at the same site. Name the missing ID and the required pi-ai version (`>=0.82.1` for `claude-opus-5`). Fire it once per registration, not per query, and do not throw — a missing row must degrade, not break startup. Note that "once per registration" is per **module load**: `MODELS` is built at module scope (`src/index.ts:123`), and pi-subagents reload the module (the re-registration guard at `src/index.ts:102-115` exists for exactly this reason), so each subagent load re-emits the diagnostic. That repetition is acceptable; if literal once-per-process is wanted, gate it with the same `Symbol.for()` pattern the stream guard uses. This is the only defence that runs in a user's install; see "Missing-row detection at runtime is required".
...
2. Add `max: "max"` to `REASONING_TO_EFFORT` as the fallback for unresolved models, and leave every existing entry — **including `xhigh: "max"`** — unchanged. The effort table below depends on `xhigh → max` surviving as the fallback for models whose catalog map defines no `xhigh` (Opus 4.6, Sonnet 4.6, Haiku 4.5) and for unresolved raw model IDs; do not "correct" the fallback's `xhigh` to `"xhigh"`.
...
15. The section 2 change 7 diagnostic fires when a mock catalog omits `claude-opus-5`, names the ID and the required pi-ai version, stays silent when every ID resolves, and does not throw in either case. Mock-driven, so it belongs with assertion 12. Note that the existing `tests/unit-models.mjs` case "silently drops IDs missing from pi-ai (no fallback)" needs updating: the drop stays silent in the returned array but is no longer silent to the operator. Also plan for test-output noise: several existing `tests/unit-models.mjs` cases build models from partial mock lists, and once the diagnostic lives in (or next to) `buildModels()` those cases will print warnings into test output. Either wrap every partial-list case with the shared `console.error` capture helper from assertion 3, or place the diagnostic at the registration call site in `src/index.ts` rather than inside `buildModels()` so mock-driven tests stay quiet by construction.
