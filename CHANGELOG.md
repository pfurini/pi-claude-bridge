...
- **Add: isolate Claude Code configuration** — run all bridge-managed Claude Code subprocesses and sessions under `~/.pi/agent/claude` by default, add the validated `provider.claudeConfigDir` override, preserve Pi paths in forwarded AGENTS.md content, keep isolation variables child-local, and add authenticated path-isolation coverage with a fail-fast auth preflight.
